...
const ShootingStarsBackground: React.FC<ShootingStarsBackgroundProps> = React.memo(({ isDarkMode }) => {
  ...
  return (
    <div className={`fixed inset-0 w-full h-full overflow-hidden bg-gradient-to-b from-[${backgroundFrom}] to-[${backgroundTo}] transition-colors duration-300`}>
      <div className="night absolute inset-0 w-full h-full">
        {Array.from({ length: 20 }).map(() => (
          <div key={Math.random().toString(36).slice(2, 11)} className={`shooting_star`} />
        ))}
      </div>
      <style>{`
        .night {
        ...
        .shooting_star {
          ...
          border-radius: 999px;
          filter: drop-shadow(0 0 3px ${starColor});
